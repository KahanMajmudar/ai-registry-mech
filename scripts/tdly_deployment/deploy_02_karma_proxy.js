...
async function main() {
	...
	console.log(
		'You are signing the following transaction: KarmaProxy.connect(EOA).deploy()'
	)
	const gasPrice = ethers.utils.parseUnits(gasPriceInGwei, 'gwei')
	const KarmaProxy = await ethers.getContractFactory('KarmaProxy')
	const karmaProxy = await KarmaProxy.connect(EOA).deploy(
		karmaAddress,
		proxyData,
		{ gasPrice }
	)
	...
	if (parsedData.contractVerification) {
		...
		execSync(
			'npx hardhat verify --constructor-args scripts/deployment/verify_02_karma_proxy.js --network ' +
				providerName +
				' ' +
				karmaProxy.address,
			{ encoding: 'utf-8' }
