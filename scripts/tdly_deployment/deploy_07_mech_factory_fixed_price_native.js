...
async function main() {
	...
	console.log(
		'You are signing the following transaction: MechFactoryFixedPriceNative.connect(EOA).deploy()'
	)
	const gasPrice = ethers.utils.parseUnits(gasPriceInGwei, 'gwei')
	const MechFactoryFixedPriceNative = await ethers.getContractFactory(
		'MechFactoryFixedPriceNative'
	)
	const mechFactoryFixedPriceNative =
		await MechFactoryFixedPriceNative.connect(EOA).deploy(
	...
	parsedData.mechFactoryFixedPriceNativeAddress =
		mechFactoryFixedPriceNative.address
	...
	if (parsedData.contractVerification) {
		...
		execSync(
			'npx hardhat verify --constructor-args scripts/deployment/verify_07_mech_factory_fixed_price_native.js --network ' +
				providerName +
				' ' +
				mechFactoryFixedPriceNative.address,
			{ encoding: 'utf-8' }
