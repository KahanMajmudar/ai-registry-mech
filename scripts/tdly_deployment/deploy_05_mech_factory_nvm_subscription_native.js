...
async function main() {
	...
	EOA = new ethers.Wallet(privateKey, provider)
	// EOA address
	...
	console.log(
		'You are signing the following transaction: MechFactoryNvmSubscriptionNative.connect(EOA).deploy()'
	)
	const gasPrice = ethers.utils.parseUnits(gasPriceInGwei, 'gwei')
	const MechFactoryNvmSubscriptionNative = await ethers.getContractFactory(
		'MechFactoryNvmSubscriptionNative'
	)
	const mechFactoryNvmSubscriptionNative =
		await MechFactoryNvmSubscriptionNative.connect(EOA).deploy(
	...
	parsedData.mechFactoryNvmSubscriptionNativeAddress =
		mechFactoryNvmSubscriptionNative.address
	...
	if (parsedData.contractVerification) {
		...
		execSync(
			'npx hardhat verify --constructor-args scripts/deployment/verify_05_mech_factory_nvm_subscription_native.js --network ' +
				providerName +
				' ' +
				mechFactoryNvmSubscriptionNative.address,
			{ encoding: 'utf-8' }
