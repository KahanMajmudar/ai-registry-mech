...
async function main() {
	...
	console.log(
		'You are signing the following transaction: MechMarketplace.connect(EOA).deploy()'
	)
	const gasPrice = ethers.utils.parseUnits(gasPriceInGwei, 'gwei')
	const MechMarketplace = await ethers.getContractFactory('MechMarketplace')
	const mechMarketplace = await MechMarketplace.connect(EOA).deploy(
	...
	if (parsedData.contractVerification) {
		...
		execSync(
			'npx hardhat verify --constructor-args scripts/deployment/verify_03_mech_marketplace.js --network ' +
				providerName +
				' ' +
