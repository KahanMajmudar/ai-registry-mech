...
async function main() {
	...
	console.log(
		'You are signing the following transaction: MechFactoryFixedPriceToken.connect(EOA).deploy()'
	)
	const gasPrice = ethers.utils.parseUnits(gasPriceInGwei, 'gwei')
	const MechFactoryFixedPriceToken = await ethers.getContractFactory(
		'MechFactoryFixedPriceToken'
	)
	const mechFactoryFixedPriceToken = await MechFactoryFixedPriceToken.connect(
		EOA
	).deploy(mechMarketplaceProxyAddress, { gasPrice })
	...
	parsedData.mechFactoryFixedPriceTokenAddress =
		mechFactoryFixedPriceToken.address
	...
	if (parsedData.contractVerification) {
		...
		execSync(
			'npx hardhat verify --constructor-args scripts/deployment/verify_09_mech_factory_fixed_price_token.js --network ' +
				providerName +
				' ' +
				mechFactoryFixedPriceToken.address,
			{ encoding: 'utf-8' }
