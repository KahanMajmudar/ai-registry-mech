...
async function main() {
	...
	console.log(
		'You are signing the following transaction: BalanceTrackerNvmSubscriptionNative.connect(EOA).deploy()'
	)
	const gasPrice = ethers.utils.parseUnits(gasPriceInGwei, 'gwei')
	const BalanceTrackerNvmSubscriptionNative = await ethers.getContractFactory(
		'BalanceTrackerNvmSubscriptionNative'
	)
	// TODO Put real drainer, now just HomeMediator address
	const balanceTrackerNvmSubscriptionNative =
		await BalanceTrackerNvmSubscriptionNative.connect(EOA).deploy(
	...
	console.log(
		'Contract address:',
		balanceTrackerNvmSubscriptionNative.address
	)
	...
	if (parsedData.contractVerification) {
		...
		execSync(
			'npx hardhat verify --constructor-args scripts/deployment/verify_06_balance_tracker_nvm_subscription_native.js --network ' +
				providerName +
				' ' +
				balanceTrackerNvmSubscriptionNative.address,
			{ encoding: 'utf-8' }
