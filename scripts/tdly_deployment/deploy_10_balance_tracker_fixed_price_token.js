...
async function main() {
	...
	console.log(
		'You are signing the following transaction: BalanceTrackerFixedPriceToken.connect(EOA).deploy()'
	)
	const gasPrice = ethers.utils.parseUnits(gasPriceInGwei, 'gwei')
	const BalanceTrackerFixedPriceToken = await ethers.getContractFactory(
		'BalanceTrackerFixedPriceToken'
	)
	// TODO Put real drainer, now just HomeMediator address
	const balanceTrackerFixedPriceToken =
		await BalanceTrackerFixedPriceToken.connect(EOA).deploy(
	...
	parsedData.balanceTrackerFixedPriceTokenAddress =
	...
	if (parsedData.contractVerification) {
		...
		execSync(
			'npx hardhat verify --constructor-args scripts/deployment/verify_10_balance_tracker_fixed_price_token.js --network ' +
				providerName +
				' ' +
				balanceTrackerFixedPriceToken.address,
			{ encoding: 'utf-8' }
