...
async function main() {
	...
	const proxyPayload = mechMarketplace.interface.encodeFunctionData(
		'initialize',
	...
	console.log(
		'You are signing the following transaction: MechMarketplaceProxy.connect(EOA).deploy()'
	)
	const gasPrice = ethers.utils.parseUnits(gasPriceInGwei, 'gwei')
	const MechMarketplaceProxy = await ethers.getContractFactory(
		'MechMarketplaceProxy'
	)
	const mechMarketplaceProxy = await MechMarketplaceProxy.connect(EOA).deploy(
		mechMarketplaceAddress,
		proxyPayload,
		{ gasPrice }
	)
	...
	if (parsedData.contractVerification) {
		...
		execSync(
			'npx hardhat verify --constructor-args scripts/deployment/verify_04_mech_marketplace_proxy.js --network ' +
				providerName +
				' ' +
