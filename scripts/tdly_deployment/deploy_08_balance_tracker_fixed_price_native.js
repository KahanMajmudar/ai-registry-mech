...
async function main() {
	...
	console.log(
		'You are signing the following transaction: BalanceTrackerFixedPriceNative.connect(EOA).deploy()'
	)
	const gasPrice = ethers.utils.parseUnits(gasPriceInGwei, 'gwei')
	const BalanceTrackerFixedPriceNative = await ethers.getContractFactory(
		'BalanceTrackerFixedPriceNative'
	)
	// TODO Put real drainer, now just HomeMediator address
	const balanceTrackerFixedPriceNative =
		await BalanceTrackerFixedPriceNative.connect(EOA).deploy(
	...
	parsedData.balanceTrackerFixedPriceNativeAddress =
		balanceTrackerFixedPriceNative.address
	...
	if (parsedData.contractVerification) {
		...
		execSync(
			'npx hardhat verify --constructor-args scripts/deployment/verify_08_balance_tracker_fixed_price_native.js --network ' +
				providerName +
				' ' +
				balanceTrackerFixedPriceNative.address,
			{ encoding: 'utf-8' }
